...
- 2026-07-08 追加除外: 論争 (culture war) クラスタ。事件性がなくても賛否が割れる
  ジェンダー/身体プライバシー/学校運用是非/ママ戦争系は、医療従事者ペルソナが
  科学的に正しい所感を書いても粘着を呼ぶ (川崎市学校健診ポストで実証)。
  誤爆で良質ネタを落とす損失 << 炎上1回の対応工数、の非対称性で攻めた除外にする。
"""
...
NEGATIVE_BANK: list[str] = [
    ...
    "事件", "事故死", "心肺停止", "重体", "重傷",
    "異物混入",
    # --- 2026-07-08 追加: 論争 (culture war) クラスタ ---
    # ジェンダー・性差 (賛否割れの定番。医療的に正しい所感でも粘着対象になる)
    "ジェンダー", "フェミ", "性差", "性別", "男女", "LGBT", "トランスジェンダー",
    "同性", "夫婦別姓", "女性専用", "男性専用",
    # 身体・プライバシー (学校健診の運用是非など)
    "脱衣", "上半身", "下着", "裸", "視触診", "身体測定", "着替え", "更衣",
    "プライバシー", "水着", "学校健診", "健康診断",
    # 論争マーカー (見出しにこれがある時点で分断ネタ)
    "賛否", "物議", "炎上", "波紋", "批判", "反発", "疑問の声", "論争", "違和感",
    "抗議",
    # 事案・処分・係争 (事件未満でも当事者がいる)
    "事案", "不祥事", "隠蔽", "処分", "懲戒", "書類送検", "提訴", "訴訟",
    "裁判", "判決", "謝罪", "撤回",
    # ママ戦争定番 + 医療論争
    "母乳", "無痛分娩", "専業主婦", "反ワクチン", "接種義務", "副反応",
]
