...
amazon-navi-brain#56 (2026-09-20) での再訪:
  ...
  型が混ざっていた:
    ...
    - A-3 / A-5 … サイトのトラフィックがさらに減り、閾値が再び大きすぎた
      (#18 と同じ「閾値側」の型)。detect_cannibalization.py /
      detect_orphan_pages.py の既定値を #56 で再較正した。
  ...
  再較正した A-3 / A-5 をいつ「妥当だった」と判断するか:
    ...
    スナップショットで両検出器とも 0 件週が出ない値まで下げてあるので、
    トラフィックがさらに減らない限り 4 週内に鳴らないはずというのが検証可能な予測。
