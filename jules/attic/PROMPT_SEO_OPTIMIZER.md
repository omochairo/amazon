...
- **検索意図の差別化（最重要・CTR 改善 #2717）**: `title_variants` の 3 案を、全商品で同一になりがちな「〇〇の口コミ・最安値・対象年齢を3サイト横断で徹底比較」という定型の言い換えだけにしない。**その商品固有の検索意図**を捉えて差別化すること。商品名（指名クエリ）は必ず含めつつ、商品ごとに異なる「読者が本当に知りたい角度」を **冒頭〜20字以内**（SP 表示が切れる前）に織り込む:
  - ブランドの所在国・メーカー（例: 「mamimami home どこの国」型の検索 →「どこの国のブランド？」を前面に）
  - 安全性・素材・認証（食品衛生法・CE・EN71 等／舐めても安全か）
  - 似た商品との違い・選び分け（競合が多いブロック系などは「〇〇と何が違う？」）
  - 具体的な対象年齢・利用シーン（「1歳の誕生日に」「指先の発達に」等）

  入力（`*.json` / `*.enrichment.json`）の `brand` / `certifications` / `competitive_analysis` / `persona_fit` / `cons` 等から、その商品で最も差別化になるシグナルを 1 つ選び **primary** に据える。3 案は互いに異なる意図を狙う（同じ角度の言い換えにしない）。定型句（「口コミ・最安値」「3サイト横断で比較」）はキーワード網羅のため使用可だが、**全案がそれだけにならないこと**。
...
1b. [ ] 3案が互いに異なる検索意図を捉えており、全案が「口コミ・最安値」定型の言い換えになっていない。primary は商品固有の意図（所在国・安全性・違い・対象年齢シーン等）を冒頭20字以内に出している（#2717）
