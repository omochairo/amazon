...
`gemini-3.8-flash-low`）。probe で成立条件を潰したうえで #6591 で実装した。
**「実装前に潰した前提」と「実装後に実データで確かめたこと」を分けて書いてある。**
...
probe の結論どおり `summary_url_balanced` を本番に入れた。

- `build_antigravity_prompt` を「注意点必須 + 各行に出典 URL」に差し替え
- `resolve_source_urls`: 本文中の URL を 302 で解決し、**自社ドメイン /
  200 以外 / 検索結果ページ**を落として `source_urls` に入れる
- 本文からは URL を除いて gemma に渡す (grounding redirect は 1 本 300 字超)
- `source_url` は **空のまま**。このレーンは複数の出典から合成した要約なので、
  行ごとの帰属を名乗ると偽の精度になる。出所は集合として `source_urls` に持つ

### gemma の呼び出し回数は変えていない

1 行 1 candidate に分ければ行ごとの帰属が取れるが、gemma のコールが 1 -> 4 に
増え、短文になることで entailment の通過率も変わる。**どちらも omochairo 機
からは測れない** (gemma は K8 側のみ) ので、v1 では candidate を 1 本のまま
にした。行分割は gemma を実測できる場所で別途判断する。

### 実データでの確認 (2026-09-06)

`B00000J048` / `B00000DMD2` で実際に通したところ、フィルタが実測で発動した:

```
WARNING 出典 URL が自社ドメイン (https://navi.omcha.jp/products/b00000dmd2/) — 循環になるので捨てる
WARNING 出典 URL が自社ドメイン (https://navi.omcha.jp/brands/raaningurisooshizu/) — 循環になるので捨てる
WARNING 出典 URL が 404 (https://supportivecaregivers.com/itemgoods/15173358.htm) — 作文か失効とみなして捨てる
```

**自社記事が実際に出典として返ってきていた。** しかも 1 本は当該 ASIN そのものの
記事。循環は仮説ではなく実在する。

残った出典は `ec.bornelund.co.jp` (公式) / `review.rakuten.co.jp` (レビュー) /
`item.rakuten.co.jp` / `oceans.tokyo.jp` など。本文にも
`【注意点・難点】` の行が入っている。

### 入れなかったフィルタ: 英語圏サイト

`target.com` などが残る。日本語の口コミ素材としては価値が低いが、**TLD や言語で
機械的に落とすと日本語で書かれた .com を巻き込む**。判別の当てが無いまま
フィルタを足すと取りこぼしの方が痛いので v1 では入れない。
必要になったら「残った URL のドメイン分布」を実測してから決める。

## 測っていないもの

- **ローカル gemma への要約移譲**。当初構想の後半。~~gemma は K8 ワーカー側にしか
  無く omochairo 機からは叩けない~~ → **これは誤り。ssh のローカルポート
  フォワード 1 本で母艦から叩ける**（2026-09-06 実測。手順は
  `ANTIGRAVITY_MODEL_BENCH.md` の「下流の歩留まり」節）。ただし probe の結果、
  agy から抜粋は取れないと分かったので、**この構想は前提から作り直しが要る** —
  経路が開いたことは、この構想が成立することを意味しない。
- **過去に収集済みの experience.json が自社記事由来かどうか。** 当時の
  `source_url` が空なので遡って判定できない。今後の収集分は `source_urls` で
  判定できるが、既存データは分からないままになる。
