...
情報が乏しいマイナー商品などで根拠を書けない場合は `score_rationale` を**省略してかまいません**（v5 で任意化、quality_gate も欠如時は skip）。ただし**記載する場合は必ず 3 件以上・各 `reason` 10字以上**としてください（中途半端な配列は NG 判定されます）。
...
5. [ ] `faq` が3項目以上ある
6. [ ] `ivs_detail.score_rationale`（任意）— 記載する場合は3件以上・各 reason 10字以上。書けない商品は省略可
...
セルフチェック後、`validate_json.py`（スキーマ）と `quality_gate.py`（品質）の
**両方が pass したら、確認を求めずにそのまま commit → PR 作成まで進めてください。**
「この内容で PR を開いてよいか」を都度尋ねる必要はありません。

ただし以下は **データ偽装にあたるため禁止**です（gate を黙らせる目的で
事実と異なる値を書かない）:

- 価格・URL・在庫など raw 由来の値を、gate を通すために 0 / 空文字 / `is_search: true`
  へ書き換える
- マッチした ASIN の実価格を捏造・改変する

`quality_gate.py` が fail した場合の正しい対処:

- **`check_no_reseller_pricing` が fail**（Amazon が cross-source 比 1.30x 超）→
  まず **その cross-source マッチが本当に同一商品か**を疑う。型番（例 E3209）が
  Amazon タイトルにあって相手側タイトルに無ければ**別 SKU の誤マッチ**であり、
  その価格は記事に載せるべきでない。誤マッチなら当該プラットフォームの価格を
  そもそも embed しない（=記事生成を止めて報告）。同一商品で seller が第三者
  リセラーなら、その ASIN は `data/asin_blocklist.json` 候補として**記事化を中止**し報告する。
- いずれの場合も、**値を偽装して auto-merge を通すことは選択肢に含めない**。
  判断に迷う fail は、回避せずそのまま停止して人間に質問してよい。
